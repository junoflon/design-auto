...
# 업종별 디자인 컨벤션 (브랜드 정보의 "업종" 필드 참고)
- 뷰티/코스메틱: 여백 많이, 파스텔/뉴트럴 톤, 세리프 액센트, 제품샷 중심, "성분/효능" 데이터 강조
- 헬스/건강기능식품: 신뢰감(톤다운된 색), 인증/임상데이터 시각화, 차트 적극, 의학적 뉘앙스
- 식품/F&B: 따뜻한 톤, 음식/원물 클로즈업, 손글씨 액센트 가능, 식감 어휘
- 패션/잡화: 에디토리얼 레이아웃, 모델 컷 중심, 미니멀 타이포, 큰 여백
- IT/SaaS: 다크모드 옵션, 그라디언트, 코드/UI 스크린샷 mockup, 성능 수치 강조
- 교육: 진도/단계 시각화, 친근한 톤, 결과/후기 강조, 가이드 일러스트
- 리빙/홈: 실내 라이프스타일컷, 따뜻한 우드/베이지 톤, 사용 시나리오

# 가격대별 디자인 차별화
- 가성비/대중형: 정보 dense, 가격 강조 큼, CTA 색 밝고 강함
- 중가/메인스트림: 균형 잡힌 정보/여백 비율
- 프리미엄: 여백 luxe, 세련된 세리프 헤드라인, 채도 낮은 컬러, 그림자 미세
- 럭셔리/하이엔드: 극도의 여백, 모노크롬+1포인트 컬러, 큰 헤드라인, 본문 미니멀
...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    const htmlCode = await injectAIImages(rawHtml, assetOutDir, `/output/${type}/${assetId}_${versionId}`, brand);
...
- 업종: ${brand.industry || '미지정'}
- 이미지 무드: ${brand.imageMood || '미니멀 스튜디오'} ← 모든 <img data-ai-gen>의 data-prompt 끝에 이 무드 키워드를 영문으로 자동 반영해야 함
- 가격대 포지셔닝: ${brand.priceTier || '중가/메인스트림'} ← 디자인 분위기(여백, 컬러 채도, 폰트 weight)에 반영` : '';
...
const MOOD_TO_EN = {
  '미니멀 스튜디오': 'minimal studio photography, clean white background, soft shadows, product-centric composition',
  '따뜻한 라이프스타일': 'warm lifestyle photography, golden hour natural light, cozy atmosphere, candid moment',
  '에디토리얼 매거진': 'editorial magazine style, sophisticated composition, fashion magazine quality, refined lighting',
  '자연광 내추럴': 'natural daylight, airy and bright, organic textures, soft diffused light',
  '팝/비비드 컬러풀': 'vibrant pop colors, bold saturated palette, playful composition, high energy',
  '럭셔리 시네마틱': 'luxury cinematic style, dramatic lighting, deep shadows, premium texture, film grain',
  '테크 미래적': 'futuristic tech aesthetic, cool tones, sleek surfaces, neon accents, sci-fi atmosphere'
};

const TIER_TO_EN = {
  '가성비/대중형': 'approachable mass-market feel',
  '중가/메인스트림': 'mainstream quality feel',
  '프리미엄': 'premium high-end feel',
  '럭셔리/하이엔드': 'ultra-luxury hi-end aesthetic'
};

async function generateOneImage({ prompt, aspect }, outDir, urlPrefix, idx, brand) {
  const size = ASPECT_TO_SIZE[aspect] || '1024x1024';
  const filename = `ai-img-${String(idx + 1).padStart(2, '0')}.png`;
  const filepath = path.join(outDir, filename);

  // Append brand mood + tier + safety constraints to every prompt
  const moodEn = MOOD_TO_EN[brand?.imageMood] || MOOD_TO_EN['미니멀 스튜디오'];
  const tierEn = TIER_TO_EN[brand?.priceTier] || TIER_TO_EN['중가/메인스트림'];
  const finalPrompt = `${prompt}. Style: ${moodEn}. Quality: ${tierEn}. No text, no letters, no logos, no watermarks in image.`;
...
async function injectAIImages(html, outDir, urlPrefix, brand) {
  ...
  console.log(`[ai-img] generating ${placeholders.length} images... (mood=${brand?.imageMood || 'default'})`);
  const results = await Promise.allSettled(
    placeholders.map((p, i) => generateOneImage(p, outDir, urlPrefix, i, brand))
  );
